  ...
  <view class="content" v-if="isContentShow">
    ...
    <view class="card">
      <tui-card :image="card.payment.img" :title="card.payment.title">
        <template slot="body">
          <view class="tui-default">
            <fui-input foces v-model="form.price" label="酬劳" type="digit" required textRight placeholder="请输入您愿意支付的酬劳" />
          </view>
        </template>
      </tui-card>
    </view>
      ...
      <tui-card :image="card.photos.img" :title="card.photos.title" :tag="card.photos.tag">
        <template slot="body">
          <view class="tui-default photo">
            <uni-file-picker :image-styles="photoStyle" v-model="form.localphotos" return-type="object" fileMediatype="image" mode="grid" @select="select" @progress="progress" @success="success" @fail="fail" @delete="deletePhoto">
